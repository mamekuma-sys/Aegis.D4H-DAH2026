...
- 새 final-SHA clean build와 이미지 검증은 PR remediation Task 5(최종 SHA 검증)에서 수행할 예정이다. 이 문서가 포함된 커밋의 SHA는 문서에 기록하지 않고, handoff 커밋 이후 PR 검증 기록에 남긴다.
...
- 원 감사 계획 Task 2: attacker target/DNS 범위 강화
- 원 감사 계획 Task 3: defender 물리 송신 E2E latency 관측
- 원 감사 계획 Task 4: bounded TCP flow 재조립과 immutable verdict snapshot 연결
- 원 감사 계획 Task 5: 필수 worker watchdog
...
## PR remediation Task 5 — 최종 SHA 인계 검증

PR remediation Task 5에서 전체 회귀·layout·skeleton·Compose 검사를 수행하고, 최종 remediation SHA로 attacker/defender `linux/amd64` 이미지를 clean-build 및 inspect한다. 두 이미지의 revision label이 최종 SHA와 일치하는지 확인한 뒤 PR 검증 기록에 결과를 남긴다.
