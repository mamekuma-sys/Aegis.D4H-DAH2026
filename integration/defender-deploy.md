...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-15-full-corpus-hardening`, `drop_capable_rules=9`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-15-full-corpus-hardening,
# drop_capable_rules=9, demotions=[] 를 확인한다.
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-15-full-corpus-hardening`, `drop_capable_rules=9`, `demotions=[]`가 정상이다. TEAM1 전체 PCAP에서 검증한 request-line rule 네 개와 canonical HTTP 의미 rule 다섯 개만 차단 권한이 있고 기존 및 병합된 13개 SHADOW rule은 차단 권한이 없다.
