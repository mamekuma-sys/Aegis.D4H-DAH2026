...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-21-p3-r8-harden`, `drop_capable_rules=31`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-21-p3-r8-harden,
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-21-p3-r8-harden`, `drop_capable_rules=31`, `demotions=[]`가 정상이다. 캡처로 검증한 HTTP·gRPC 의미 규칙까지 31개가 ACTIVE이며, 정상/공격 PCAP이 없는 L3 MQTT·RTSP 후보와 일반 휴리스틱은 SHADOW다. query 없는 `/portal/feedback`과 플래그 유출이 연결되지 않은 `systemConfig` 조회는 ACTIVE 차단에 포함하지 않는다.
