...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-21-p4-r11-cross-layer-hardening`, `drop_capable_rules=40`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-21-p4-r11-cross-layer-hardening,
# drop_capable_rules=40, demotions=[] 를 확인한다.
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-21-p4-r11-cross-layer-hardening`, `drop_capable_rules=40`, `demotions=[]`가 정상이다. 캡처와 라운드 로그로 검증한 HTTP·gRPC·MQTT·RTSP, L4 Exchange/ROS/mission, portal·bridge·imagery 경로와 보호 서비스 응답 유출 규칙까지 40개가 ACTIVE다.
