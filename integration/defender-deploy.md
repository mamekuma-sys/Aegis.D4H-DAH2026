...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-21-finals-portmap`, `drop_capable_rules=12`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-21-finals-portmap,
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-21-finals-portmap`, `drop_capable_rules=12`, `demotions=[]`가 정상이다. L1(8080/8082)·L2·L3 정밀 규칙 9개에 더해 SatDiag(`9000`) Tail/Export 2개와 P2-R3 GraphQL(`8082`) `missionAudit`/`systemConfig` 차단 1개가 ACTIVE이고, 나머지 휴리스틱은 SHADOW다.
