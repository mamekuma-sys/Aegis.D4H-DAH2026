...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-21-p4-r11-l4-wire-hardening`, `drop_capable_rules=37`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-21-p4-r11-l4-wire-hardening,
# drop_capable_rules=37, demotions=[] 를 확인한다.
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-21-p4-r11-l4-wire-hardening`, `drop_capable_rules=37`, `demotions=[]`가 정상이다. 캡처와 라운드 로그로 검증한 HTTP·gRPC·MQTT·RTSP, L4 Exchange/ROS/mission 경로 및 보호 서비스 응답의 raw·escaped flag 유출 규칙까지 37개가 ACTIVE다. 광역 portal `service_id` 규칙은 SLA 보호를 위해 SHADOW이고, 증명된 SSTI만 ACTIVE다.
