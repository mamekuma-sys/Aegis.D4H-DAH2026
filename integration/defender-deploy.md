...
5. 공식 스켈레톤 라이브 스모크 — 공격·방어를 동시에 기동하고, 시작 로그가 `policy_source=active`, `bundle_id=defender-2026-08-21-p2r4-evidence-fix`, `drop_capable_rules=14`, `demotions=[]`인지 확인한다. PACKET→VERDICT·HEARTBEAT·재연결을 확인한다.
...
# 시작 로그에서 policy_source=active, bundle_id=defender-2026-08-21-p2r4-evidence-fix,
...
- **시작 로그**: `policy_source=active`, `bundle_id=defender-2026-08-21-p2r4-evidence-fix`, `drop_capable_rules=14`, `demotions=[]`가 정상이다. HTTP 정밀 규칙 11개에 SatDiag(`9000`) Tail/Export 2개와 P2-R3 GraphQL(`8082`) `missionAudit` 차단 1개를 더한 14개가 ACTIVE이고, 나머지 휴리스틱은 SHADOW다. query 없는 `/portal/feedback`과 플래그 유출이 연결되지 않은 `systemConfig` 조회는 ACTIVE 차단에 포함하지 않는다.
