...
2026-08-21 팀에 전달된 최신 본선 안내를 구조화한 매핑입니다. 원본 공지 자체를
...
- 팀 총 budget: `$1360`
- 비용 환산: 당일 공지의 모델별 입력·출력·cache 1M token 단가를 JSON에 기록
...
공격 기본 모델은 `gpt-5.6-sol`(팀별 583K TPM / 583 RPM)이고, 일시적 실패 시
`gpt-5.6-terra`를 한 번 시도합니다. 방어 기본 모델은 기존 `gpt-4o-mini`(팀별 6.5M TPM /
65K RPM)를 유지합니다. 코드의 라운드별 호출 상한은 공격 48회(fallback 포함), 방어
20회이며 quota보다 낮습니다. 이 호출 상한은 비용 상한이 아니므로 팀 공용 비용 ledger는
별도로 관리해야 합니다.
