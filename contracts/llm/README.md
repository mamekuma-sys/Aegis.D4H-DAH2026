...
공격·방어 기본 모델은 모두 `gpt-5.6-sol`(팀별 583K TPM / 583 RPM)입니다. 공격은
일시적 실패 시 `gpt-5.6-terra`를 한 번 시도합니다. 코드의 라운드별 호출 상한은 공격
48회(fallback 포함), 방어 20회이며 quota보다 낮습니다. 이 호출 상한은 비용 상한이
아니므로 팀 공용 비용 ledger는 별도로 관리해야 합니다.
